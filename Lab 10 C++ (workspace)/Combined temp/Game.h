...
namespace Game {
    ...
    public:
    public:
			Wall():GameObject(new OneColorTexture(Color()), Vector3D(), Vector3D(), Vector3D()){}
        //pass in corners as bottom left, bottom right, top right, top left
        //(counter clockwise
        Wall(Color color_, const Vector3D& corner1, const Vector3D& corner2,const Vector3D& corner3,const Vector3D& corner4): GameObject(new OneColorTexture(color_), corner1+(corner2-corner1).scale(.5)+(corner4-corner1).scale(.5), (corner4-corner1).scale(.5), (corner2-corner1).scale(.5)){}
        ...
        RayIntersect intersect(const Vector3D& rayPos, const Vector3D& rayDirection) const {
            ...
            rayInt.didIntersect = (s>=-1 && s<=1 && t>=-1 && t<=1) && d>0 && (d!=-infin && d!=infin);
