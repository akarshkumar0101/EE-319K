...
static Color pixelBuffer[DISPLAY_WIDTH*DISPLAY_HEIGHT];
...
void solidifyDisplay(){
    //solidPixelBuffer = pixelBuffer1;
    //changingPixelBuffer = pixelBuffer1;
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (key == GLFW_KEY_UP && (action == GLFW_REPEAT||action==GLFW_PRESS)){
        Game::moveForward(translateSpeed);
    }
    if (key == GLFW_KEY_DOWN && (action == GLFW_REPEAT||action==GLFW_PRESS)){
        Game::moveForward(-translateSpeed);
    }
    if (key == GLFW_KEY_RIGHT && (action == GLFW_REPEAT||action==GLFW_PRESS)){
        Game::moveSide(translateSpeed);
    }
    ...
    if (key == GLFW_KEY_W && (action == GLFW_REPEAT||action==GLFW_PRESS)){
        Game::rotateVertical(rotateSpeed);
    }
    if (key == GLFW_KEY_S && (action == GLFW_REPEAT||action==GLFW_PRESS)){
        Game::rotateVertical(-rotateSpeed);
    }
    if (key == GLFW_KEY_A && (action == GLFW_REPEAT||action==GLFW_PRESS)){
        Game::rotateHorizontal(rotateSpeed);
    }
...
void drawPixel(int x, int y) {
    ...
    Color color = pixelBuffer[y*DISPLAY_WIDTH+x];
...
void setColor(int x, int y, uint16_t color){
    //changingPixelBuffer[y*DISPLAY_WIDTH+x].color = color;
    
    if(x>=0 && x<DISPLAY_WIDTH && y>=0 && y <DISPLAY_HEIGHT)
        pixelBuffer[y*DISPLAY_WIDTH+x].color = color;
}
void setColor(int x, int y, const Color& color){
    setColor(x,y,color.color);
}
...
void drawBitMap(int x, int y, const uint16_t* image, int bitmapWidth, int bitmapHeight){
    for(int px = x;px<x+bitmapWidth;px++){
        for(int py = y;py<y+bitmapHeight;py++){
            setColor(px, py, image[(py-y)*bitmapWidth+px-x]);
        }
    }
}
