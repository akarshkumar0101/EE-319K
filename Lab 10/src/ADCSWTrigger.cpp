...
//  SYSCTL_RCGC0_R |= 0x00010000; // 1) activate ADC0 (legacy code)
  ...
  delay = SYSCTL_RCGCGPIO_R;      // 2) allow time for clock to stabilize
  delay = SYSCTL_RCGCGPIO_R;
  GPIO_PORTE_DIR_R &= ~0x3C;      // 3) make PE4 PE5, PE2, PE3 input
  GPIO_PORTE_AFSEL_R |= 0x3C;     // 4) enable alternate function on PE4 PE5 PE2, PE3
  GPIO_PORTE_DEN_R &= ~0x3C;      // 5) disable digital I/O on PE4 PE5 PE2, PE3
                                  // 5a) configure PE4 as ?? (skip this line because PCTL is for digital only)
  GPIO_PORTE_PCTL_R = GPIO_PORTE_PCTL_R&0xFF00FFFF;
  GPIO_PORTE_AMSEL_R |= 0x3C;     // 6) enable analog functionality on PE4 PE5 PE2, PE3
  ADC0_PC_R &= ~0xF;              // 8) clear max sample rate field
  ADC0_PC_R |= 0x1;               //    configure for 125K samples/sec
  ADC0_SSPRI_R = 0x3210;          // 9) Sequencer 3 is lowest priority
  ADC0_ACTSS_R &= ~0x0004;        // 10) disable sample sequencer 2
  ADC0_EMUX_R &= ~0x0F00;         // 11) seq2 is software trigger
  ADC0_SSMUX2_R = 0x1089;         // 12) set channels for SS2
...
void ADC_In89(uint32_t data[4]){ 
  ...
  data[1] = ADC0_SSFIFO2_R&0xFFF;  // 3B) read second result
	...
	data[3] = ADC0_SSFIFO2_R&0xFFF;  // 3B) read second result
  ADC0_ISC_R = 0x0004;             // 4) acknowledge completion
}
