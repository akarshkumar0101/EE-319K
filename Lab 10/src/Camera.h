...
namespace Camera{
    ...
    void establishViewPort(){
        ...
        //viewPortBvect = right.cross(dir).normalize().scale(tan(viewAngleY/2.0f)).scale(viewportDistance);
        
        //tan 40deg = 0.8390996312
        viewPortAvect = right.normalize().scale(0.8390996312f).scale(viewportDistance);
        viewPortBvect = right.cross(dir).normalize().scale(0.8390996312f).scale(viewportDistance);
    }
    
    Vector3D getPixelRayWithViewPort(float x, float y){
        return viewPortPos+viewPortAvect.scale(x)+viewPortBvect.scale(y)-pos;
    }
    ...
    bool isInDirectionOfCamera(const Vector3D& point){
        return dir.dot((point-pos))>0;
    }
